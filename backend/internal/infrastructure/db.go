...
func CreateTable(handler *Sqlhandler) (err error) {
	_, err = handler.Execute("CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, firstname VARCHAR(255) NOT NULL, lastname VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL UNIQUE, password VARCHAR(255) NOT NULL, salt VARCHAR(255) NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) DEFAULT CHARSET=utf8mb4 ENGINE=InnoDB;")
	if err != nil {
		return
	}

	_, err = handler.Execute("CREATE TABLE IF NOT EXISTS tasks (id SERIAL PRIMARY KEY, user_id BIGINT UNSIGNED DEFAULT NULL, title VARCHAR(255) NOT NULL, is_done TINYINT(1) NOT NULL DEFAULT 0, description TEXT, deadline DATETIME, elapsed INTEGER NOT NULL DEFAULT 0, reward FLOAT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE SET NULL ON UPDATE CASCADE) DEFAULT CHARSET=utf8mb4 ENGINE=InnoDB;")
	if err != nil {
		return
	}
